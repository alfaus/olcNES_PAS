...
function T6502.GetFlag(f: Byte): Boolean;
begin
  Result := (status and f) > 0;
end;

procedure T6502.SetFlag(f: Byte; v: Boolean);
begin
  if v then
    status := status or f
  else
...
function T6502.ReadByte(adr: Word): Byte;
begin
  // In normal operation "read only" is set to false. This may seem odd. Some
	...
	// devices on the bus
  Result := TBus(bus).ReadByte(adr, False);
end;

// Writes a byte to the bus at the specified address
procedure T6502.WriteByte(adr: Word; d: Byte);
begin
  TBus(bus).WriteByte(adr, d);
end;
...
function T6502._RTS: Byte;
begin
  Inc(stkp);
  pc := ReadByte($0100 + stkp);
  Inc(stkp);
  pc := pc or (Word($0100 + stkp) shl 8);

  Inc(pc);
  Result := 0;
end;

function T6502._SBC: Byte;
var
  value: Word;
begin
  Fetch;
...
begin
  ...
  begin
    ...
    begin
      AssignFile(logfile, 'olc6502.txt');
      Append(logfile);
    end else
    begin
      WriteLn(logfile, Format(
        '%10d:%02d PC:%04X %s A:%02X X:%02X Y:%02X %s%s%s%s%s%s%s%s STKP:%02X',
        [
...
function T6502.Complete: Boolean;
begin
  Result := cycles = 0;
end;

procedure T6502.ConnectBus(n: TObject);
begin
  bus := n;
end;
...
function T6502.Disassemble(nStart, nStop: Word): TMapStrings;
var
  addr: Cardinal;
  line_addr: Word;
...
begin
  ...
  begin
    ...
    op := TBus(bus).ReadByte(addr, True); Inc(addr);
    ...
    begin
      value := TBus(bus).ReadByte(addr, True); Inc(addr);
      sInst += '#$' + hexStr(value, 2) + ' {IMM}';
    end
    else if lookup[op].addrmode = @_ZP0 then
    begin
      lo_byte := TBus(bus).ReadByte(addr, True); Inc(addr);
      hi_byte := $00;
    ...
    begin
      lo_byte := TBus(bus).ReadByte(addr, True); Inc(addr);
      hi_byte := TBus(bus).ReadByte(addr, True); Inc(addr);
    ...
    begin
      value := TBus(bus).ReadByte(addr, True); Inc(addr);
      sInst += '$' + hexStr(value, 2) + ' [$' + hexStr(addr + value, 4) + '] {REL}';
    end;
