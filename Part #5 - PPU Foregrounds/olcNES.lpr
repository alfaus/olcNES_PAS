...
begin
  ...
  begin
    ...
    nes.controller[0] := $00;
    if SfmlKeyboardIsKeyPressed(sfKeyX) then nes.controller[0] := nes.controller[0] or $80;
    if SfmlKeyboardIsKeyPressed(sfKeyZ) then nes.controller[0] := nes.controller[0] or $40;
    if SfmlKeyboardIsKeyPressed(sfKeyA) then nes.controller[0] := nes.controller[0] or $20;
    if SfmlKeyboardIsKeyPressed(sfKeyS) then nes.controller[0] := nes.controller[0] or $10;
    if SfmlKeyboardIsKeyPressed(sfKeyUp) then nes.controller[0] := nes.controller[0] or $08;
    if SfmlKeyboardIsKeyPressed(sfKeyDown) then nes.controller[0] := nes.controller[0] or $04;
    if SfmlKeyboardIsKeyPressed(sfKeyLeft) then nes.controller[0] := nes.controller[0] or $02;
    if SfmlKeyboardIsKeyPressed(sfKeyRight) then nes.controller[0] := nes.controller[0] or $01;
    ...
    for i:=0 to 25 do
    begin
      sp := hexStr(i, 2) + ': (' + IntToStr(nes.ppu.pOAM[i * 4 + 3]) +
           ', ' + IntToStr(nes.ppu.pOAM[i * 4 + 0]) + ') ' +
           'ID: ' + hexStr(nes.ppu.pOAM[i * 4 + 1], 2) +
           ' AT: ' + hexStr(nes.ppu.pOAM[i * 4 + 2], 2);
      DrawString(516, 72 + i * 10, sp, SfmlWhite);
    end;
